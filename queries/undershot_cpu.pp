...
query "vpa_cpu_undershot_deployment" {
  sql = <<-EOQ
    ...
    WHERE
        cpu_convert(r.target_cpu) > cpu_convert(d.requests_cpu);
  EOQ
...
query "vpa_cpu_undershot_statefulset" {
  sql = <<-EOQ
...
query "vpa_cpu_undershot_daemonset" {
  sql = <<-EOQ
